...
class Paypal {
    ...
    function __construct($credentials = array()) {
        ...
        $credentials['sandbox_status'] = $this->CI->config->item('paypal_debug');
        ...
        if (isset($credentials['sandbox_status'])) {
            ...
            if ($this->sandbox_flag === false) {
                // PROD
                $this->API_username = $this->CI->config->item('paypal_prod_username');
                $this->API_password = $this->CI->config->item('paypal_prod_password');
                $this->API_signature = $this->CI->config->item('paypal_prod_signature');
                $this->API_endpoint = "https://api-3t.paypal.com/nvp";
                $this->paypal_url = "https://www.paypal.com/cgi-bin/webscr?cmd=_express-checkout&token=";
            } else {
                // DEV
                $this->API_username = $this->CI->config->item('paypal_dev_username');
                $this->API_password = $this->CI->config->item('paypal_dev_password');
                $this->API_signature = $this->CI->config->item('paypal_dev_signature');
    ...
    function set_ec($ec_data = array(), $custom_data = false) {
        ...
        if ($custom_data === false) {
            $nvpstr .= "&LOCALECODE=fr_FR";
            if (isset($ec_data['landing'])) {
                $nvpstr .= "&LANDINGPAGE=" . urlencode($ec_data['landing']);
            }
            if (isset($ec_data['email'])) {
                $nvpstr .= "&EMAIL=" . urlencode($ec_data['email']);
            }
            if (isset($ec_data['firstname'])) {
                $nvpstr .= "&PAYMENTREQUEST_0_SHIPTONAME=" . urlencode($ec_data['firstname'] . ' ' . $ec_data['lastname']);
            }
            if (isset($ec_data['address'])) {
                $nvpstr .= "&PAYMENTREQUEST_0_SHIPTOSTREET=" . urlencode($ec_data['address']);
            }
            if (isset($ec_data['city'])) {
                $nvpstr .= "&PAYMENTREQUEST_0_SHIPTOCITY=" . urlencode($ec_data['city']);
            }
            if (isset($ec_data['zipcode'])) {
                $nvpstr .= "&PAYMENTREQUEST_0_SHIPTOZIP=" . urlencode($ec_data['zipcode']);
            }
            if (isset($ec_data['phone'])) {
                $nvpstr .= "&PAYMENTREQUEST_0_SHIPTOPHONENUM=" . urlencode($ec_data['phone']);
            }
            if (isset($ec_data['country_code'])) {
                $nvpstr .= "&PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE=" . urlencode($ec_data['country_code']);
            }
            if (isset($ec_data['logo'])) {
                $nvpstr .= "&LOGOIMG=" . urlencode($ec_data['logo']);
            }
            if (isset($ec_data['brandname'])) {
                $nvpstr .= "&BRANDNAME=" . urlencode($ec_data['brandname']);
            }
            ...
            if (isset($ec_data['tax_amount'])) {
                $tax_amount = round((float) $ec_data['tax_amount'], 2);
